...
export async function GET(request:Request){ const secret=process.env.CRON_SECRET; const auth=request.headers.get('authorization'); if(!secret||auth!==`Bearer ${secret}`) return Response.json({error:'Não autorizado'},{status:401}); try{const url=new URL(request.url);const rawBatch=Number(url.searchParams.get('batch')??0);const rawTotal=Number(url.searchParams.get('total')??1);const batch=Number.isInteger(rawBatch)?rawBatch:0;const totalBatches=Number.isInteger(rawTotal)?rawTotal:1;const options={batch,totalBatches};const [rss,youtube,bluesky,mastodon,lemmy,nostr]=await Promise.all([collectActiveRssSources(options),collectYoutubeMonitors(options),collectBlueskyMonitors(options),collectMastodonMonitors(options),collectLemmyMonitors(options),collectNostrMonitors(options)]);const youtubeComments=await collectYoutubeComments(options);return Response.json({ok:true,batch,totalBatches,executedAt:new Date().toISOString(),results:{rss,youtube,youtubeComments,bluesky,mastodon,lemmy,nostr}});}catch(error){console.error('[cron/collect]',error);return Response.json({ok:false,error:error instanceof Error?error.message:'Falha na coleta'},{status:500});} }
