...
export async function GET(){
 try{
  ...
  const [todayCount,weekCount,monthCount,sourcesCount,monitorsCount,recent]=await Promise.all([
  ...
  const failure=[todayCount,weekCount,monthCount,sourcesCount,monitorsCount,recent].find(x=>x.error); if(failure?.error) throw failure.error;
  return Response.json({connected:true,metrics:{today:todayCount.count??0,week:weekCount.count??0,month:monthCount.count??0,sources:sourcesCount.count??0,monitors:monitorsCount.count??0},mentions:recent.data??[],updatedAt:now.toISOString()});
 }catch(error){console.error('[api/dashboard]',error);return Response.json({connected:false,metrics:{today:0,week:0,month:0,sources:0,monitors:0},mentions:[],error:'Banco ainda não conectado.'},{status:503});}
}
