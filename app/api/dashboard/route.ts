...
type MonitorMention={id:string;created_at:string;relevance_score:number|null;matched_text:string|null;articles:{title:string;description:string|null;url:string;published_at:string|null;sources:{name:string}|null;sentiment_analysis:Array<{sentiment:string;score:number}>}|null};
type MonitorRow={id:string;name:string;status:string;created_at:string;monitor_keywords:Array<{keyword:string;type:string}>;mentions:MonitorMention[]};
export async function GET(){
 try{
  ...
  const [todayCount,weekCount,monthCount,sourcesCount,sourceRows,monitorsCount,monitorRows,recent]=await Promise.all([
   ...
   db.from('monitors').select('id,name,status,created_at,monitor_keywords(keyword,type),mentions(id,created_at,relevance_score,matched_text,articles(title,description,url,published_at,sources(name),sentiment_analysis(sentiment,score)))').order('created_at',{ascending:false}),
  ...
  const failure=[todayCount,weekCount,monthCount,sourcesCount,sourceRows,monitorsCount,monitorRows,recent].find(x=>x.error); if(failure?.error) throw failure.error;
