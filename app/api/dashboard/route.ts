...
export async function GET(){
 try{
  ...
  const [todayCount,weekCount,monthCount,sourcesCount,sourceRows,projectsRows,monitorsCount,monitorRows,recent]=await Promise.all([
   ...
   db.from('mentions').select('id,matched_text,relevance_score,created_at,articles(title,description,url,published_at,sources(name)),monitors(id,name,project_id)').order('created_at',{ascending:false}).limit(50)
  ]);
