...
export async function GET(){
 try{
  ...
  const [todayCount,weekCount,monthCount,sourcesCount,sourceRows,projectsRows,monitorsCount,monitorRows,recent]=await Promise.all([
   ...
   db.from('monitors').select('id,project_id,name,status,created_at,monitor_keywords(keyword,type)').eq('status','active').order('created_at',{ascending:false}),
  ...
  const metricEntries=await Promise.all(rows.map(async row=>{
   const [todayResult,weekResult,monthResult,totalResult]=await Promise.all([
    db.from('mentions').select('id',{count:'exact',head:true}).eq('monitor_id',row.id).gte('created_at',today.toISOString()),
    db.from('mentions').select('id',{count:'exact',head:true}).eq('monitor_id',row.id).gte('created_at',d7.toISOString()),
    db.from('mentions').select('id',{count:'exact',head:true}).eq('monitor_id',row.id).gte('created_at',d30.toISOString()),
    db.from('mentions').select('id',{count:'exact',head:true}).eq('monitor_id',row.id)
   ]);
