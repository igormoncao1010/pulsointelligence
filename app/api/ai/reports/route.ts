...
export async function POST(request:Request){
  ...
  try{
    ...
    const response=await fetch('https://router.huggingface.co/v1/chat/completions',{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify({model:currentModel,messages:[{role:'system',content:system},{role:'user',content:prompt}],temperature:.2,max_tokens:1200}),signal:AbortSignal.timeout(50000)});
    const raw=await response.json() as {choices?:Array<{message?:{content?:string}}>;usage?:Record<string,number>;error?:{message?:string}|string};if(!response.ok)throw new Error(typeof raw.error==='string'?raw.error:raw.error?.message||`Hugging Face respondeu HTTP ${response.status}`);const content=raw.choices?.[0]?.message?.content;if(!content)throw new Error('O Hugging Face não retornou conteúdo.');const result=parseReport(content);
    const {data:saved,error:saveError}=await db.from('ai_monitor_reports').update({status:'complete',result,token_usage:raw.usage??null,completed_at:new Date().toISOString(),error_message:null}).eq('id',reportId).select('*').single();if(saveError)throw saveError;return Response.json({report:saved,cached:false});
